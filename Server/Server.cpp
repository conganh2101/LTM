...
typedef struct _FILE_TRANSFER_PROPERTY {
	char		fileName[100];
...
void HandleIo(ULONG_PTR key, BUFFER_OBJ *buf, HANDLE CompPort, DWORD BytesTransfered, DWORD error)
{
	...
	if (buf->operation == OP_ACCEPT)
	{
		...
		if (clientobj)
		{
			...
			if (rcvMess->opcode == fileNameDownload) {
				


				strcpy_s(clientobj->fileTransfer.fileName, "D://server/");
				strcat_s(clientobj->fileTransfer.fileName, rcvMess->payload);
				fprintf(stderr, "%s\n", clientobj->fileTransfer.fileName);

				if (isFileExists(clientobj->fileTransfer.fileName)) {
					FILE *file;
					//Open file
					file = fopen(clientobj->fileTransfer.fileName, "rb");
					if (!file)
					{
						fprintf(stderr, "Unable to open file %s", clientobj->fileTransfer.fileName);
						return;
					}

					//Get file length
					fseek(file, 0, SEEK_END);
					clientobj->fileTransfer.fileLen = ftell(file);
					fseek(file, 0, SEEK_SET);
					clientobj->fileTransfer.nLeft = clientobj->fileTransfer.fileLen;
					clientobj->fileTransfer.idx = 0;


					clientobj->fileTransfer.fileBuffer = (char*)malloc(clientobj->fileTransfer.fileLen + 1);
					if (!clientobj->fileTransfer.fileBuffer)
					{
					...
					fread(clientobj->fileTransfer.fileBuffer, clientobj->fileTransfer.fileLen, 1, file);
					fclose(file);//***************************

					MD5 md5;
					sendMessage.opcode = md5Code;
                    strcpy_s(sendMessage.payload, md5.digestFile(clientobj->fileTransfer.fileName));
					sendMessage.length = strlen(md5.digestFile(clientobj->fileTransfer.fileName));
				}
				else
				{
					sendMessage.opcode = noFileExist;
					strcpy_s(sendMessage.payload, clientobj->fileTransfer.fileName);
					sendMessage.length = strlen(clientobj->fileTransfer.fileName);
				}
			...
			else if (rcvMess->opcode == fileNameUpload) {
				strcpy_s(clientobj->fileTransfer.fileName, "D://server/");
				strcat_s(clientobj->fileTransfer.fileName, rcvMess->payload);
				fprintf(stderr, "%s\n", clientobj->fileTransfer.fileName);
				...
				if (!isFileExists(clientobj->fileTransfer.fileName))
				{
					clientobj->fileTransfer.file = fopen(clientobj->fileTransfer.fileName, "wb");
					if (!clientobj->fileTransfer.file)
					{
						fprintf(stderr, "Unable to open file ");
						return;
					}
					sendMessage.opcode = uploadFile;
					strcpy_s(sendMessage.payload, clientobj->fileTransfer.fileName);
					sendMessage.length = strlen(clientobj->fileTransfer.fileName);
				}
				else 
				{
					sendMessage.opcode = fileExist;
					strcpy_s(sendMessage.payload, clientobj->fileTransfer.fileName);
					sendMessage.length = strlen(clientobj->fileTransfer.fileName);
				}
	...
	else if (buf->operation == OP_READ)
	{
		...
		if (BytesTransfered > 0)
		{
			...
			if (BytesTransfered == sizeof(message))
			{
				message *rcvMess;
				rcvMess = (message *)buf->buf;
				if (rcvMess->opcode == downloadFile)
				{
					fprintf(stderr, "succes:");
					message sendMessage;

					sendMessage.opcode = sendingData;
					if (sockobj->fileTransfer.nLeft > BUFF_SIZE)
					{
						for (int i = 0; i < BUFF_SIZE; i++) {
							sendMessage.payload[i] = sockobj->fileTransfer.fileBuffer[sockobj->fileTransfer.idx + i];
						}
						sendMessage.length = BUFF_SIZE;
					}
					else
					{
						for (int i = 0; i < sockobj->fileTransfer.nLeft; i++) {
							sendMessage.payload[i] = sockobj->fileTransfer.fileBuffer[sockobj->fileTransfer.idx + i];
						}
						sendMessage.length = sockobj->fileTransfer.nLeft;
					}
					sendMessage.offset = sockobj->fileTransfer.idx;
					fprintf(stderr, "[%d:%d]\n", sockobj->fileTransfer.nLeft, sendMessage.length);

					sockobj->fileTransfer.nLeft -= sendMessage.length;
					sockobj->fileTransfer.idx += sendMessage.length;
				...
				else if (rcvMess->opcode == sendingData)
				{
					if (rcvMess->length == 0) {
						
						fclose(sockobj->fileTransfer.file);
						MD5 md5;
						if (strcmp(md5.digestFile(sockobj->fileTransfer.fileName), sockobj->fileTransfer.digest) == 0)
						{
							fprintf(stderr, "succes\n");
							message sendMessage;

							sendMessage.opcode = success;
							strcpy_s(sendMessage.payload, sockobj->fileTransfer.fileName);
							sendMessage.length = strlen(sockobj->fileTransfer.fileName);
						...
						else
						{
							...
							if (remove(sockobj->fileTransfer.fileName) != 0)
							{
							...
							sendMessage.opcode = fileCorrupted;
							strcpy_s(sendMessage.payload, sockobj->fileTransfer.fileName);
							sendMessage.length = strlen(sockobj->fileTransfer.fileName);
					...
					else
					{
						...
						fseek(sockobj->fileTransfer.file, rcvMess->offset, SEEK_SET);
						fwrite(rcvMess->payload, 1, rcvMess->length, sockobj->fileTransfer.file);
				...
				else if (rcvMess->opcode == md5Code)
				{
					fprintf(stderr, "MD5");
					strcpy_s(sockobj->fileTransfer.digest, rcvMess->payload);
	...
	else if (buf->operation == OP_WRITE)
	{
		...
		if (sockobj->bClosing == FALSE)
		{
			...
			if (queueMessage->opcode == sendingData)
			{
				message sendMessage;
			...
			else if (queueMessage->opcode == md5Code)
			{
				fprintf(stderr, "checking download");
				buf->sock = sockobj;
				PostRecv(sockobj, buf);
			}
			else if (queueMessage->opcode == uploadFile) 
			{
